...
function reactivarProducto(idProducto) {
  const confirmar = confirm(
    "¿Deseas volver a activar este producto para que aparezca en el catálogo público?",
  );
  if (confirmar) {
...
if (btnVerOrdenes) {
  btnVerOrdenes.addEventListener("click", function () {
    ...
    if (viendoPedidos) {
      ...
      btnAgregarProd.style.display = "none"; // Ocultamos el botón de agregar producto
...
function cambiarEstadoEnvio(idOrden, nuevoEstado) {
  if (
    confirm(
      `¿Estás seguro de marcar este pedido como '${nuevoEstado}'? El cliente verá esta actualización.`,
    )
  ) {
    fetch(`http://localhost:8080/orden/update-status/${idOrden}`, {
      ...
      .then((data) => {
        if (data.success) {
          cargarPedidosRecibidos(); // Recargar la lista para ver el nuevo botón
        } else {
